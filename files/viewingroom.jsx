...
function formatDateRange(start, end) {
  ...
  const full = (d) => d.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
...
function InquireBtn({ small, artist, title, artworkId, onHold }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <button onClick={onClose} style={{
        ...
        color:'#FFFFFF', fontSize:22, cursor:'pointer', fontFamily:"'Replica', sans-serif",
        letterSpacing:'0.08em', opacity:0.6, transition:'opacity 0.15s', padding:'4px 8px',
      }}
      ...
      <div style={{
        ...
        fontSize:10, letterSpacing:'0.14em', textTransform:'uppercase', color:'rgba(255,255,255,0.45)',
      }}>
        {String(idx+1).padStart(2,'0')} / {String(total).padStart(2,'0')}
      </div>
      {/* Image */}
      <div onClick={e => e.stopPropagation()} style={{
        maxWidth:'90vw', maxHeight:'90vh', display:'flex', alignItems:'center', justifyContent:'center',
      }}>
      ...
      {idx > 0 && (
        <button onClick={e => { e.stopPropagation(); setIdx(i => i - 1); }} style={{
          ...
          fontSize:28, fontFamily:'serif', transition:'color 0.15s', padding:'8px 12px',
        }}
          onMouseEnter={e => e.currentTarget.style.color='#FFFFFF'}
          onMouseLeave={e => e.currentTarget.style.color='rgba(255,255,255,0.55)'}>←</button>
      )}
      {/* Next */}
      {idx < total - 1 && (
        <button onClick={e => { e.stopPropagation(); setIdx(i => i + 1); }} style={{
...
// ── INSTALLATION VIEWS ────────────────────────────────────────────────────────
function InstallationViews({ images, isMobile }) {
  ...
  return (
    <>
      ...
      <section id="installation-views" style={{
        maxWidth:1200, margin:'0',
        padding: isMobile ? '40px 20px 60px' : '30px 48px 80px',
      }}>
        <div className="subtitle-gray" style={{ marginBottom:24 }}>Installation Views</div>
        <div style={{
          display:'grid',
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#FFFFFF' }}>
      <header style={{ maxWidth:1200, margin:'0', padding: isMobile ? '32px 20px 24px' : '56px 48px 36px' }}>
        <a href="https://diez.gallery" target="_blank" rel="noopener noreferrer" style={{ display:'inline-block', marginBottom:20, marginLeft:-4 }}>
            ...
            {room.installViews?.length > 0 && (
              <a href="#installation-views"
                ...
                style={{ display:'inline-block', fontSize:12,
                  ...
                  textDecoration:'none', transition:'color 0.15s', fontWeight:400,
                  borderBottom:'1px solid #000000', paddingBottom:1, width:'fit-content' }}
                onMouseEnter={e => { e.target.style.color='#666666'; e.target.style.borderBottomColor='#666666'; }}
                onMouseLeave={e => { e.target.style.color='#000000'; e.target.style.borderBottomColor='#000000'; }}>
                ↓ Installation Views
              </a>
            )}
          </div>
      ...
      <footer style={{ padding: isMobile ? '40px 20px' : '48px 48px',
        ...
        <a href="https://airtable.com/appkTmFvjmDLOQS4p/pageOJZwubCWxkwCs/form" target="_blank" rel="noopener noreferrer"
        ...
        <a href="https://instagram.com/diez.gallery" target="_blank" rel="noopener noreferrer"
  ...
  return (
    <div onClick={onSelect} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{
        ...
        gap: isMobile ? '16px' : '48px', padding:'25px 0', borderBottom:'1px solid #000000',
      ...
      <div style={{ position:'relative', overflow:'hidden', borderRadius:1,
        ...
        {work.imageUrl ? (
          ...
          <div style={{ width:'100%', aspectRatio:'4/3', display:'flex',
      ...
      <div style={{ display:'flex', flexDirection:'column', justifyContent:'center', paddingTop:4 }}>
        ...
        <div style={{ marginTop:20 }}>
          {work.price && showPrice && (
            <div className="price" style={{ marginBottom:8 }}>
              {`€ ${Number(work.price).toLocaleString('de-DE')}`}
            </div>
          )}
          <Dot status={work.status} available={work.available}/>
        </div>
        {showInquire && (
          <div style={{ marginTop:16 }}>
            <InquireBtn small={false} artist={work.artist} title={work.title} artworkId={work.id} onHold={onHold}/>
          </div>
        )}
      </div>
    </div>
...
// ── DETAIL SPLIT ──────────────────────────────────────────────────────────────
function DetailSplit({ works, workIdx, onBack, onPrev, onNext, onJump }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => { if (e.key === 'Escape') setZoomActive(false); };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [zoomActive]);

  // Reset to main image when changing artwork
  useEffect(() => { setDetailIndex(-1); setZoomActive(false); }, [workIdx]);
  ...
  const currentLabel = detailIndex === -1
    ? (hasDetails ? 'Click to see detail' : null)
    : (detailIndex < work.detailUrls.length - 1 ? 'Click for next detail' : 'Click to see full view');
    ...
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', overflow:'hidden', background:'#FFFFFF' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
        ...
        <div style={{ display:'flex', alignItems:'center', gap: isMobile ? 16 : 28 }}>
          <a href="https://diez.gallery" target="_blank" rel="noopener noreferrer" style={{ display:'inline-flex', alignItems:'center' }}>
            <img src="/files/logo.png" alt="diez" style={{ height: isMobile ? 18 : 22, width:'auto', display:'block' }}/>
          </a>
      ...
      <div style={{ flex:1, display:'flex', flexDirection: stackLayout ? 'column' : 'row',
        ...
        <div style={{ flex: stackLayout ? 'none' : '0 0 60%', background:'#FFFFFF',
          ...
          {work.imageUrl ? (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center',
              ...
              onMouseMove={(e) => {
                ...
                setZoomPos({ x, y, visible, w: r.width, h: r.height });
              }}
              ...
              <img src={currentSrc}
                ...
                style={stackLayout
                  ? { width:'100%', height:'auto', maxHeight: isTablet ? '60vh' : '80vh', objectFit:'contain', display:'block', animation:'fadeIn 0.35s ease both' }
                  : { maxWidth:'100%', maxHeight:'calc(100vh - 100px)', width:'auto', height:'auto', objectFit:'contain', display:'block', animation:'fadeIn 0.35s ease both' }
                }/>
              {zoomActive && zoomPos.visible && imgRef.current && (
                <div style={{
                  position:'absolute',
              ...
              {!stackLayout && (
                <button onClick={(e) => { e.stopPropagation(); setZoomActive(z => !z); }}
                  ...
                  style={{ position:'absolute', bottom:16, right:16,
                    width:34, height:34, borderRadius:'50%', border:'1px solid rgba(0,0,0,0.2)',
                    background: zoomActive ? '#000000' : 'rgba(255,255,255,0.9)',
                    color: zoomActive ? '#FFFFFF' : '#444444',
                    cursor:'pointer', fontSize:14, fontFamily:'serif',
                    display:'flex', alignItems:'center', justifyContent:'center',
                    boxShadow:'0 2px 8px rgba(0,0,0,0.08)', transition:'all 0.18s', zIndex:6 }}>
                  ⌕
                </button>
              )}
              {!stackLayout && zoomActive && (
                <div style={{ position:'absolute', bottom:22, right:60, zIndex:6,
                  fontSize:9, letterSpacing:'0.1em', textTransform:'uppercase', color:'#666666',
                  background:'rgba(255,255,255,0.9)', padding:'4px 8px', borderRadius:2,
                  pointerEvents:'none' }}>
          ...
          {!stackLayout && [{fn:onPrev,dis:idx===0,lbl:'←',pos:'left'},{fn:onNext,dis:idx===total-1,lbl:'→',pos:'right'}].map(a => (
        ...
        {/* Info panel */}
        <div style={{ flex: stackLayout ? '0 0 auto' : '0 0 40%', overflowY:'auto',
          borderLeft: stackLayout ? 'none' : '1px solid #000000', borderTop: stackLayout ? '1px solid #000000' : 'none',
          ...
          <div className="divider" style={{ padding: isMobile ? '16px 16px' : '20px 36px 28px', margin:0,
            ...
            <div>
              ...
              <div style={{ marginTop:5 }}><Dot status={work.status} available={work.available}/></div>
            </div>
            {showInquire && <InquireBtn artist={work.artist} title={work.title} artworkId={work.id} onHold={onHold}/>}
          </div>
          <div style={{ padding: isMobile ? '0 16px 100px' : isTablet ? '0 36px 60px' : '0 36px 28px', display:'flex', gap:8, flexWrap:'wrap',
            overflowX: isMobile ? 'auto' : 'visible' }}>
            {works.map((w, i) => (
              <div key={w.id} onClick={() => onJump(i)}
...
function App() {
  ...
  async function load(slug) {
    try {
      ...
      setRoom({
        ...
        dates: formatDateRange(vr['Start Date'], vr['End Date']), intro: vr['Introduction'] || '',
      ...
      if (deepId) {
        ...
        if (i >= 0) {
          setWorkIdx(i);
          setScreen('detail');
          history.replaceState({ screen: 'detail', workIdx: i }, '', window.location);
        }
      }
  ...
  const openWork = useCallback((i) => {
    ...
    if (w) {
      ...
      history.pushState({ screen: 'detail', workIdx: i }, '', url);
    }
  }, [works]);
  ...
  useEffect(() => {
    ...
    const url = new URL(window.location);
    url.searchParams.set('work', w.id);
    history.replaceState({ screen: 'detail', workIdx }, '', url);
  ...
  useEffect(() => {
    if (screen !== 'detail') { lastTrackedIdx.current = -1; return; }
  ...
  if (screen === 'landing') {
    return (<div ref={landingScrollRef} style={{ height:'100vh', overflowY:'auto' }}><Landing room={room} works={works} onSelect={openWork}/></div>);
  }
  return (
    <div style={{ height:'100vh', overflow:'hidden' }}>
      <DetailSplit works={works} workIdx={workIdx}
        onBack={() => history.back()} onPrev={() => go(-1)} onNext={() => go(1)}
